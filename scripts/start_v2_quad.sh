...
PIDS="$PIDS $!"
ros2 launch mavros px4.launch fcu_url:="udp://:14542@localhost:14559" namespace:="/x500_2" tgt_system:="3" > "$LOG_DIR/mavros_drone2.log" 2>&1 & 
PIDS="$PIDS $!"
ros2 launch mavros px4.launch fcu_url:="udp://:14543@localhost:14560" namespace:="/x500_3" tgt_system:="4" > "$LOG_DIR/mavros_drone3.log" 2>&1 & 
PIDS="$PIDS $!"
...
#PIDS="$PIDS $!"
cd ~/PX4-Autopilot && PX4_SYS_AUTOSTART=4001 PX4_GZ_MODEL_POSE="0,3" PX4_SIM_MODEL=gz_x500 ./build/px4_sitl_default/bin/px4 > "$LOG_DIR/px4_drone0.log" 2>&1 &
PIDS="$PIDS $!"
...
PIDS="$PIDS $!"
cd ~/PX4-Autopilot && PX4_SYS_AUTOSTART=4001 PX4_GZ_MODEL_POSE="0,-3" PX4_SIM_MODEL=gz_x500 ./build/px4_sitl_default/bin/px4 -i 2 > "$LOG_DIR/px4_drone2.log" 2>&1 & 
PIDS="$PIDS $!"
cd ~/PX4-Autopilot && PX4_SYS_AUTOSTART=4001 PX4_GZ_MODEL_POSE="-3,0" PX4_SIM_MODEL=gz_x500 ./build/px4_sitl_default/bin/px4 -i 3 > "$LOG_DIR/px4_drone3.log" 2>&1 & 
PIDS="$PIDS $!"
...
ros2 launch virtual_gps virtual_gps_v2_takeoff.launch.py &
PIDS="$PIDS $!"
...
ros2 param set /x500_2/param MPC_XY_VEL_MAX 2.0
ros2 param set /x500_3/param MPC_XY_VEL_MAX 2.0
