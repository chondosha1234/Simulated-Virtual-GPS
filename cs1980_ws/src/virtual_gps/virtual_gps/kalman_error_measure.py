...
class KalmanErrorMeasureNode(Node):
    
    def __init__(self):
        super().__init__('kalman_error_measure')
        ...
        self.error_publisher = self.create_publisher(Float32, f'/{self.robot_name}/kalman_error', 1)
        ...
        self.gps_subscription = self.create_subscription(TransformStamped, f'/{self.robot_name}/filtered_gps', self.gps_callback, 1)
    ...
    def timer_callback(self):
        ...
        error = self.calculate_error(self.filter_gps_pose, self.robot_actual_pose)
        self.counter += 1

        # here for testing purposes 
        self.get_logger().info(f'kalman error distance calc: {error}')
